	...
	ESX.UI.Menu.Open(
		...
		css = 'vestiaire',
			...
			elements = {
				...
				{label = _U('water_clothes'), value = 'water_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'water_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'water_actions_menu'
...
function OpenWaterActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'water_actions',
		{
		css = 'vestiaire',
			title    = 'Traitement Eau',
			align    = 'top-left',
			elements = elements
		},
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'water', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 241.17, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'water', vehicleProps)
		...
		local elements = {
			{label = 'Fourgon Employé',  value = 'boxville'},
			{label = 'Fourgon Employé 2',  value = 'bison'},
			{label = 'Fourgon Patron',  value = 'burrito'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 241.17, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					SetVehicleCustomPrimaryColour(vehicle, 0, 255, 255) --Peinture primaire
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
...
function OpenMobileWaterActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_water_actions',
		{
		css = 'metier',
			title    = 'Traitement Eau',
			align    = 'top-left',
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_water', 'Water', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_waterjob:getStockItems', function(items)
		local elements = {}
		...
		{css='metier',
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_waterjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_waterjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		{css='metier',
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_waterjob:putStockItems', itemName, count)
...
AddEventHandler('esx_waterjob:hasEnteredMarker', function(zone)
	if zone == 'EauFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'water'  then
		CurrentAction     = 'eau_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'TraitementEau' and PlayerData.job ~= nil and PlayerData.job.name == 'water'  then
		CurrentAction     = 'eau_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'TraitementBouteille' and PlayerData.job ~= nil and PlayerData.job.name == 'water'  then
		CurrentAction     = 'bouteille_traitement'
		CurrentActionMsg  = _U('press_traitement_raffine')
		CurrentActionData = {zone= zone}
	end	
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'water'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'WaterActions' and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
		CurrentAction     = 'water_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
...
AddEventHandler('esx_waterjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'EauFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
		TriggerServerEvent('esx_waterjob:stopHarvest')
	end  
	if (zone == 'TraitementEau' or zone == 'TraitementBouteille') and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
		TriggerServerEvent('esx_waterjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'water' then
		TriggerServerEvent('esx_waterjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.WaterActions.Pos.x, Config.Zones.WaterActions.Pos.y, Config.Zones.WaterActions.Pos.z)
		...
		AddTextComponentString("Power And Water")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'water' then

		for k,v in pairs(Config.Zones)do
			if v.Type == 1 then
				local blip2 = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z)

				SetBlipSprite (blip2, 620)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'water' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_waterjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_waterjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'water' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'eau_harvest' then
					TriggerServerEvent('esx_waterjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'eau_traitement' then
					TriggerServerEvent('esx_waterjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'bouteille_traitement' then
					TriggerServerEvent('esx_waterjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_waterjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'water_actions_menu' then
					OpenWaterActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'water', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'water' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileWaterActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)