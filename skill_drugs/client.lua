...
RegisterNetEvent('skill_drugs:outlawNotify')
AddEventHandler('skill_drugs:outlawNotify', function(alert)
...
RegisterNetEvent("skill_drugs:UsableItem")
AddEventHandler("skill_drugs:UsableItem",function()
	local player = PlayerPedId()
	if IsPedInAnyVehicle(player) then
		exports['progressBars']:startUI(4000, "Connection de l'usb vers l'appareil")
	else
...
function HackingMinigame(success)
    ...
    if success then
		Citizen.Wait(350)
		ChooseDrugMenu()
		ESX.ShowNotification("Vous avez ~g~avec succès~s~ hacker le ~b~réseau~s~")
    else
		ESX.ShowNotification("Vous avez ~g~rater le~s~ hacking du ~b~réseau~s")
...
function ChooseDrugMenu()
	...
	function(data, menu)
			...
			if data.current.value == "cancel_drug_job" then
				ESX.ShowNotification("Vous avez ~r~annulé~s~ la demande")
				menu.close()
				ClearPedTasks(player)
				FreezeEntityPosition(player,false)
			else
				TriggerServerEvent("skill_drugs:GetSelectedJob",data.current.value, data.current.BuyPrice, data.current.MinReward, data.current.MaxReward )
				Citizen.Wait(100)
				menu.close()
				ClearPedTasks(player)
				FreezeEntityPosition(player,false)
...
AddEventHandler("skill_drugs:BrowseAvailableJobs",function(spot,drugType,minReward,maxReward)
	...
	if currentID == 100 then
		ESX.ShowNotification("Aucun ~y~jobs~s~ n'est actuellement disponible, veuillez réessayer plus tard !")
	else
		SelectedID = id
		TriggerEvent("skill_drugs:startMainEvent",id,drugType,minReward,maxReward)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
	end
end)

RegisterNetEvent('skill_drugs:startMainEvent')
AddEventHandler('skill_drugs:startMainEvent', function(id,drugType,minReward,maxReward)
	local Goons = {}
	...
	TriggerServerEvent("skill_drugs:syncJobsData",Config.Jobs)
	Citizen.Wait(500)
	...
	while not JobCompleted and not StopTheJob do
		...
		if Config.Jobs[id].InProgress == true then
            ...
            if (GetDistanceBetweenCoords(coords, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z, true) > 60) and DeliveryInProgress == false then
				DrawMissionText("Atteindre le ~y~Van~s~ marqué sur votre GPS.")
			end
			...
			if DeliveryInProgress == false and (GetDistanceBetweenCoords(coords, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z, true) < 60) and (GetDistanceBetweenCoords(coords, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z, true) > 10) then
				DrawMissionText("~r~Tuez~s~ les personnes qui garde le ~y~Van~s~")
			end
			...
			if isVehicleLockPicked == false and (GetDistanceBetweenCoords(coords, selectedJob.Spot.x, selectedJob.Spot.y, selectedJob.Spot.z, true) < 10) then
				DrawMissionText("Voler et verrouiller le ~y~Van~s~.")
			end
			...
			if (GetDistanceBetweenCoords(coords, VanPosition.x, VanPosition.y, VanPosition.z, true) <= 2) and isVehicleLockPicked == false then
				DrawText3Ds(VanPosition.x, VanPosition.y, VanPosition.z, "Appuyez sur ~g~[G]~s~ pour ~y~le dévérouiller~s~.")
			...
			if isVehicleLockPicked == true and vanIsDelivered == false then
				if not InsideJobVan then
					DrawMissionText("Montez dans le ~y~Van~s~.")
				end
			end
			...
			if DeliveryInProgress == true and isVehicleLockPicked == true and vanIsDelivered == false then
				if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
					local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
					if GetEntityModel(vehicle) == GetHashKey(Config.JobVan) then
						DrawMissionText("Amenez le van à la destination indiquée sur votre GPS.")
					end
				end
			end
				...
				for k,v in pairs(Config.DeliverySpot) do
					if InsideJobVan then
						...
						if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0) and vanIsDelivered == false then
							DrawText3Ds(v.x, v.y, v.z, "Appuyez sur ~g~[E]~s~ pour ~y~livré~s~")
			...
			if DeliveryInProgress == true and vanIsDelivered == true and not drugBoxInHand and not drugsTaken then
				DrawMissionText("Prenez le paquet de drogues dans le camion.")
			end
			
			if DeliveryInProgress == true and vanIsDelivered == true and drugBoxInHand and not drugsTaken then
				DrawMissionText("Mettez le paquet de drogue dans votre véhicule de fuite.")
			end
							...
							if not IsPedInAnyVehicle(GetPlayerPed(-1), true) then
								...
								if GetEntityModel(vehicle) == GetHashKey(Config.JobVan) and not drugBoxInHand then
									...
									if Distance < 2.0 then
										DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z, "Appuyez sur ~g~[E]~s~ pour prendre ~y~le paquet~s~ du van.")
										NearJobVehicle = true
									else
								...
								elseif GetEntityModel(vehicle) ~= GetHashKey(Config.JobVan) and drugBoxInHand then
									...
									local Distance = GetDistanceBetweenCoords(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z, coords.x, coords.y, coords.z, false);
									if Distance < 2.0 then
										DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z, "Appuyez sur ~g~[E]~s~ pour mettre ~y~le paquet~s~ dans le vehicule.")
			...
			if NearOtherVehicle == true and drugBoxInHand and IsControlJustPressed(0, 38) then
                ESX.ShowNotification("~g~Job finit~s~")
				PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
                ClearPedTasks(GetPlayerPed(-1))
                DeleteEntity(attachedProp)
				TriggerServerEvent("skill_drugs:JobReward",minRewardD,maxRewardD,typeDrug)
				drugsTaken = true
				StopTheJob = true
			end
...
function LockpickJobVan(selectedJob)
	...
	if Config.PoliceNotfiyEnabled == true then
		TriggerServerEvent('skill_drugs:DrugJobInProgress',GetEntityCoords(PlayerPedId()),streetName)
	end
	...
	exports['progressBars']:startUI(7500, "Deverouillage du véhicule")
	Citizen.Wait(7500)
...
RegisterNetEvent("skill_drugs:DrugEffects")
AddEventHandler("skill_drugs:DrugEffects", function(k,v)
...
RegisterNetEvent("skill_drugs:ConvertProcess")
AddEventHandler("skill_drugs:ConvertProcess", function(k,v)
		...
		repeat
			...
			if distance < 2 and CanSellToPed(ped) and canSellDrugs and not IsPedInAnyVehicle(player, true) then
				if Config.Enable3DTextToSell then
					DrawText3Ds(pos.x, pos.y, pos.z, "Appuyez sur ~g~[H]~s~ pour vendre ~y~la drogue~s~")
				else
					ESX.ShowHelpNotification("Appuyez sur ~g~ ~INPUT_VEH_HEADLIGHT~ ~s~ pour vendre ~r~la drogue~s~")
				end
				if IsControlJustPressed(1,74) then
					...
					if chance == 1 or chance == 2 then
						...
						TriggerServerEvent("skill_drugs:sellDrugs")
					else
						chance = math.random(1,Config.CallPoliceChance)
						if chance == 1 then
							if Config.PoliceNotfiyEnabled == true then
								TriggerServerEvent('skill_drugs:DrugSaleInProgress',GetEntityCoords(PlayerPedId()),streetName)
							end
							ESX.ShowNotification("la vente a été ~r~refusé ~s~ et  ~b~la police~s~ a été averti")
						else
							ESX.ShowNotification("la vente a été ~r~refusé ~s~")	
						end
					end
...
Citizen.CreateThread(function()
	while true do
		TriggerServerEvent("skill_drugs:canSellDrugs")
		Citizen.Wait(Config.DrugSaleCooldown * 1000)
	end
end)
...
RegisterNetEvent("skill_drugs:canSellDrugs")
AddEventHandler("skill_drugs:canSellDrugs", function(soldAmount)
...
RegisterNetEvent('skill_drugs:OutlawBlipEvent')
AddEventHandler('skill_drugs:OutlawBlipEvent', function(targetCoords)
...
RegisterNetEvent("skill_drugs:syncJobsData")
